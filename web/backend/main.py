"""
FastAPI Backend for Code Quality Intelligence Agent - FINAL RAG-INTEGRATED VERSION
"""
import asyncio
import shutil
import tempfile
...
app = FastAPI(title="🤖 Code Quality Intelligence API - RAG Integrated", version="2.1.0")
...
async def run_analysis_and_build_rag(analysis_id: str, path: str, source_type: str):
    ...
    try:
        ...
        if source_type in ["github", "upload"]:
...
async def summary_query_with_context(request: InteractiveQueryRequest):
    """Handles Q&A about the final analysis REPORT."""
    try:
...
async def rag_query_with_context(request: InteractiveQueryRequest):
    """Handles Q&A directly with the codebase using the RAG index."""
    try:
        analysis = analysis_cache.get(request.analysis_id)
        if not analysis or analysis.get("status") != "completed":
            raise HTTPException(404, "Analysis context not ready or found.")
        ...
        # Create a new RAG analyzer instance for this request to ensure thread safety
